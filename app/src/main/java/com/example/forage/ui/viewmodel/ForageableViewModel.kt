...
class ForageableViewModel(
    ...
    val forageables: LiveData<List<Forageable>> = forageableDao.getAll().asLiveData()

    fun getForageableToDetail(id: Long): LiveData<Forageable> =
    ...
    fun deleteForageable(forageable: Forageable) {
        viewModelScope.launch(Dispatchers.IO) {
            forageableDao.delteForageable(forageable)
        }
    }
